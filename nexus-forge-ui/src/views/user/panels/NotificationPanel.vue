    ...
    <section class="panel" aria-labelledby="h-notifications">
        ...
        <article class="card section-card">
            ...
            <div v-for="dnd in dndList" :key="dnd.title" class="toggle-row">
                ...
                <button class="toggle" role="switch" :aria-checked="dnd.on" @click="dnd.on = !dnd.on"></button>
            </div>
        </article>
        ...
        <article class="card section-card">
            ...
            <div class="row-between export-row">
                <p class="body-sm body-muted export-desc">
                    将以 JSON 格式打包你创建的项目、评论、上传的文件与设置项。文件准备完成后会通过站内信通知,链接 7 天内有效。
                </p>
...
const dndList = reactive([
    { title: '工作时间外免打扰', desc: '每天 22:00 至次日 08:00 暂停推送与邮件,仅保留账号安全提醒。', on: true },
    { title: '会议模式', desc: '日历显示"会议中"时自动暂停,直到会议结束。', on: false },
])

const privacy = reactive([
    { title: '公开资料显示在团队成员列表', desc: '关闭后,只有你加入的工作区管理员能查看你的简介。', on: true },
    { title: '允许搜索引擎索引个人主页', desc: '关闭后,搜索引擎不会收录 qiming.tech/linwan 页面。', on: true },
    { title: '用 AI 改进产品体验', desc: '允许我们将匿名交互数据用于训练和优化推荐模型。可随时撤销。', on: false },
])
</script>
